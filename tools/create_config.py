...
def main():
    ...
    if args.dataset in ['voc']:
        ...
        for i, lineinfo in enumerate(yaml_info):
            if '  EXP_TRAIN: ' in lineinfo:
                _str_ = '  EXP_TRAIN: ("pvoc_explict_split{}_{}shot_seed0", )\n'
                yaml_info[i] = _str_.format(args.split, args.shot)
            elif '  IMP_TRAIN: ' in lineinfo:
                _str_ = '  IMP_TRAIN: ("pvoc_implicit_07_split{}_{}shot_seed0", "pvoc_implicit_12_split{}_{}shot_seed0")\n'
                yaml_info[i] = _str_.format(args.split, args.shot, args.split, args.shot)
            elif '  TRAIN: ' in lineinfo:
