...
class FastRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        num_false_negative = torch.nonzero(
            (fg_pred_classes == bg_class_ind), as_tuple=False).numel()

        num_accurate = torch.nonzero(
            (pred_classes == self.gt_classes), as_tuple=False).numel()
        fg_num_accurate = torch.nonzero(
            (fg_pred_classes == fg_gt_classes), as_tuple=False).numel()
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        self.bbox_pred = nn.Linear(input_size, num_bbox_reg_classes * box_dim, bias=False)
