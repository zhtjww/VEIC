...
PVOC_CATEGORIES = [
    ...
    {"color": [0, 80, 100], "isthing": 1, "id": 7, "name": "chair"},
    {"color": [0, 0, 192], "isthing": 1, "id": 8, "name": "cow"},
    {"color": [165, 42, 42], "isthing": 1, "id": 9, "name": "diningtable"},
    {"color": [255, 77, 255], "isthing": 1, "id": 10, "name": "dog"},
    {"color": [0, 226, 252], "isthing": 1, "id": 11, "name": "horse"},
    {"color": [182, 182, 255], "isthing": 1, "id": 12, "name": "person"},
    {"color": [0, 82, 0], "isthing": 1, "id": 13, "name": "pottedplant"},
    {"color": [120, 166, 157], "isthing": 1, "id": 14, "name": "train"},
    {"color": [197, 226, 255], "isthing": 1, "id": 15, "name": "tvmonitor"},
    {"color": [153, 69, 1], "isthing": 1, "id": 16, "name": "boat"},
    {"color": [3, 95, 161], "isthing": 1, "id": 17, "name": "cat"},
    {"color": [163, 255, 0], "isthing": 1, "id": 18, "name": "motorbike"},
    {"color": [0, 182, 199], "isthing": 1, "id": 19, "name": "sheep"},
    {"color": [183, 130, 88], "isthing": 1, "id": 20, "name": "sofa"},
]
...
def _get_pvoc_instances_meta():
    thing_ids = [k["id"] for k in PVOC_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in PVOC_CATEGORIES if k["isthing"] == 1]
...
def _get_pvoc_fewshot_instances_meta():
    ret = _get_pvoc_instances_meta()
    novel_ids = [k["id"] for k in PVOC_CATEGORIES if k["isthing"] == 1]
    novel_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(novel_ids)}
    novel_classes = PASCAL_VOC_NOVEL_CATEGORIES
    base_categories = [
        k for k in PVOC_CATEGORIES if k["isthing"] == 1 and k["name"] not in novel_classes
    ]
...
def _get_builtin_metadata(dataset_name):
    ...
    elif dataset_name == "pvoc_fewshot":
        return _get_pvoc_fewshot_instances_meta()
